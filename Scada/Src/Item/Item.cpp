...
void TItem::fDraw(QPainter* painter)
{
    Q_UNUSED(painter);
}
...
{
   //qDebug() << "TItem::mouseMoveEvent"<<mDropScale;
    if(mDropScale)
    {
...
void TItem::fCopy(TItem *ItemFrom)
{
    ...
    this->fSetStartPos(ItemFrom->fGetStartX(), ItemFrom->fGetStartY());
    this->fSetStopPos(ItemFrom->fGetStopX(), ItemFrom->fGetStopY());
    this->fSetLineWidth(ItemFrom->fGetLineWidth());
    this->fSetLineColor(ItemFrom->fGetLineColor());
    this->fSetLineStyle(ItemFrom->fGetLineStyle());
    this->fSetBackGroundColor(ItemFrom->fGetBackGroundColor());
    this->fSetSelectAble(ItemFrom->fGetSelectAble());
    this->fSetMoveAble(ItemFrom->fGetMoveAble());
    this->fSetDragAble(ItemFrom->fGetDragAble());
    this->fSetZValue(ItemFrom->fGetZValue());
}
...
double TItem::fGetCenterX()
{
    double StartX = fGetStartX();
    double StopX = fGetStopX();

    return StartX + (StopX - StartX) / 2;
}

double TItem::fGetCenterY()
{
    double StartY = fGetStartY();
    double StopY = fGetStopY();

    return StartY + (StopY - StartY) / 2;
}

double TItem::fGetCurrentStartX()
{
    return mapToScene(mStartX, mStartY).x();
}

double TItem::fGetCurrentStartY()
{
    return mapToScene(mStartX, mStartY).y();
}

double TItem::fGetCurrentStopX()
{
    return mapToScene(mStopX, mStopY).x();
}

double TItem::fGetCurrentStopY()
{
    return mapToScene(mStopX, mStopY).y();
}

double TItem::fGetCurrentCenterX()
{
    double StartX = fGetCurrentStartX();
    double StopX = fGetCurrentStopX();

    return StartX + (StopX - StartX) / 2;
}

double TItem::fGetCurrentCenterY()
{
    double StartY = fGetCurrentStartY();
    double StopY = fGetCurrentStopY();
